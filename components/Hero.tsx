...
export default function Hero({ acc, badge, cta }: { acc: string; badge: string; cta: string }) {
  ...
  return (
    <section style={{ background: "var(--bg)", minHeight: "100vh",
      display: "flex", alignItems: "center", paddingTop: 80, position: "relative", overflow: "hidden" }}>
      {/* Glow orbs — keep existing style */}
      <div className="glow-orb" style={{ width: 600, height: 600, top: -200, left: -200, position: "absolute",
        background: `radial-gradient(circle, ${acc}22 0%, transparent 70%)`, pointerEvents: "none" }} />
      <div className="glow-orb" style={{ width: 400, height: 400, bottom: -100, right: -100, position: "absolute",
        background: "radial-gradient(circle, rgba(99,140,255,0.12) 0%, transparent 70%)",
        animationDelay: "-7s", pointerEvents: "none" }} />

      <div className="container" style={{ display: "grid",
        gridTemplateColumns: "1fr 1fr", gap: "clamp(40px,6vw,80px)",
        ...
        <div>
          ...
          <div style={{ marginBottom: 24, height: 36, display: "flex", alignItems: "center" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8,
              ...
              <span style={{ fontSize: 12, fontWeight: 600, color: active.color,
          ...
          <p style={{ fontSize: "clamp(16px,1.8vw,20px)", color: "rgba(255,255,255,0.55)",
            lineHeight: 1.65, maxWidth: 480, marginBottom: 40 }}>
            VOXA makes outbound AI calls that qualify leads, handle objections, and book
            meetings — in your industry's language, in English, Hindi, or Hinglish.
          </p>

          {/* CTA */}
          <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
            <Link href="/signup" style={{ display: "inline-flex", alignItems: "center",
              gap: 8, padding: "14px 28px", borderRadius: 10, fontSize: 15, fontWeight: 600,
              background: acc, color: "#0F0F14", textDecoration: "none",
            ...
            {[["147+","Active tenants"],["6","Industries"],["<800ms","Response time"]].map(([n,l]) => (
              <div key={l}>
                <div style={{ fontSize: 20, fontWeight: 700, color: "#F3F4F6",
                  fontFamily: "var(--font-display)", letterSpacing: -0.02 }}>{n}</div>
        ...
        <div>
          ...
          <div className="glass-panel transcript-block"
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%",
                background: active.color, animation: "pulse-dot 2s infinite" }} />
              <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.08em",
                textTransform: "uppercase", color: "rgba(255,255,255,0.3)" }}>
                Live call · {active.label}
              </span>
            </div>
            ...
            {active.exampleTranscript.map((turn, i) => (
              <div key={i} style={{ display: "flex", gap: 10, marginBottom: 12 }}>
                <span className="transcript-role"
                  style={{ color: turn.role === "AI" ? active.color : "rgba(255,255,255,0.3)",
                    fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", marginRight: 8 }}>
                  {turn.role === "AI" ? "AI" : "Lead"}
                </span>
                <span style={{ color: turn.role === "AI"
                  ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.45)",
                  lineHeight: 1.55, flex: 1, fontSize: "12.5px", fontFamily: "var(--font-mono)" }}>{turn.text}</span>
              </div>
            ))}

            {/* Outcome indicator */}
            <div style={{ marginTop: 16, padding: "10px 14px",
              background: `${active.color}18`, borderRadius: 8,
              display: "flex", alignItems: "center", gap: 8,
              color: active.color, fontSize: 12, fontWeight: 500 }}>
          ...
          <div style={{ marginTop: 14, display: "flex", alignItems: "center",
            gap: 12, fontSize: 11, color: "rgba(255,255,255,0.25)" }}>
            <span>Powered by</span>
            {["Claude AI", "Vapi", "WhatsApp", "Google Calendar"].map(s => (
              <span key={s} style={{ padding: "2px 8px",
