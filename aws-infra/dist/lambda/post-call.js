...
async function getSecrets() {
    ...
    try {
        const data = await secretsClient.send(new client_secrets_manager_1.GetSecretValueCommand({ SecretId: secretArn }));
...
const handler = async (event) => {
    ...
    if (secrets.PINECONE_API_KEY)
        process.env.PINECONE_API_KEY = secrets.PINECONE_API_KEY;
    ...
    if (!calendly) {
        calendly = new calendly_1.CalendlyService(secrets.CALENDLY_TOKEN || "mock_calendly_pat_token");
    }
    if (!whatsapp) {
        whatsapp = new whatsapp_1.WhatsAppService(secrets.WHATSAPP_PHONE_ID || "109876543210", secrets.WHATSAPP_TOKEN || "mock_whatsapp_cloud_token");
    }
    ...
    for (const record of event.Records) {
        try {
            ...
            const sanitizedTranscript = (0, pii_service_1.maskPII)(transcriptText);
...
Target JSON Schema Structure:
{
  ...
  "requires_booking": boolean // True if they booked a walkthrough consult, site measurement, or showroom visit during the call.
            ...
            const rawTextResult = await (0, llm_service_1.invokeLLMWithFallback)(`Analyze the following interior design client call transcript and extract the parameters:
        
        --- TRANSCRIPT START ---
        ${sanitizedTranscript}
        --- TRANSCRIPT END ---`, systemPrompt, true // isComplex consultation analysis
            );
            ...
            try {
                ...
                await (0, memory_service_1.appendLeadEvent)(contactId, "PreferenceExtracted", {
                ...
                await (0, memory_service_1.appendLeadEvent)(contactId, "CallCompleted", {
                ...
                if (extractedData.budget === 0) {
                    await (0, memory_service_1.appendLeadEvent)(contactId, "ObjectionRaised", {
                ...
                if (extractedData.requires_booking) {
                    ...
                    await activeWhatsapp.sendTemplateMessage(customerPhone, "portfolio_and_voucher", [customerName, discussedStyle]);
