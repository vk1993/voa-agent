...
const handler = async (event) => {
    ...
    try {
        ...
        const sanitizedTranscript = (0, pii_service_1.maskPII)(transcript);
        // 1. Fetch historical replayed memory summary
        const rawMemory = await (0, memory_service_1.buildLeadContext)(contactId);
        ...
        if (intentClass === "Acknowledge") {
            ...
            llmResponse = await (0, llm_service_1.invokeLLMWithFallback)(`History:\n${compressedMemory}\n\nQuestion: "${sanitizedTranscript}"`, `You are an helpful interior sales assistant. Answer client questions simply in 1-2 sentences using this compressed catalog context: ${compressedCatalog}`, false // routes to simple gpt-4o-mini, fails over to Bedrock Haiku/Sonnet
            );
        }
        else {
            ...
            llmResponse = await (0, llm_service_1.invokeLLMWithFallback)(`Timeline Memory:\n${compressedMemory}\n\nClient Query: "${sanitizedTranscript}"`, `You are VOXA's Senior Luxury Interior Architect. Conduct a deeply personalized and professional consultation comparing budgets, materials, and soft-close structures based on this catalog context: ${compressedCatalog}. Replay client memory timeline naturally.`, true // routes to complex gpt-4o, fails over to Bedrock Sonnet
            );
        }
        return {
            statusCode: 200,
