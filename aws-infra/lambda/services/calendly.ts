...
export class CalendlyService {
   ...
   */
  async getSchedulingLink(userUri: string): Promise<string> {
    console.log(`Fetching Calendly scheduling link for user: ${userUri}`);
    try {
      // Query event types associated with this user account
      const data = await this.request(`/event_types?user=${encodeURIComponent(userUri)}`) as any;
      
      const activeEvent = data.collection?.find((et: any) => et.active === true);
      if (!activeEvent) {
   ...
   */
  async getAvailableSlots(eventTypeId: string): Promise<any[]> {
    console.log(`Resolving available availability slots for Event Type: ${eventTypeId}`);
    try {
      const data = await this.request(`/user_availability_schedules?event_type=${eventTypeId}`) as any;
   ...
   */
  async createBooking(eventUri: string, inviteeData: { email: string; name: string }): Promise<any> {
